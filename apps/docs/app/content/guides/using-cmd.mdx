...
const output = Box.combineAll([Cmd.clearScreen, Box.text("Hello, terminal!")]);
...
const leave = Box.combineAll([Cmd.altScreenLeave, Cmd.cursorShow]);
```
...
const output = pipe(
  ...
  Box.combine(Box.text("Centered title").pipe(Box.alignHoriz(Box.center1, 65)))
);
```
...
const teardown = Box.combineAll([Cmd.altScreenLeave, Cmd.cursorShow]);
