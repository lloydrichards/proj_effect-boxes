...
export const outCol = Box.renderPrettySync(pipe(
  Flex.col(
    [
      Flex.fixed(Box.text("┄ Header ┄").pipe(Box.alignHoriz(Box.center1, 80))),
      Flex.grow(Box.text("Body content goes here...").pipe(Box.pad(1, 2))),
      Flex.fixed(Box.text("┄ Footer ┄").pipe(Box.alignHoriz(Box.center1, 80))),
    ],
    10
  ),
  Box.border("rounded")
...
export const outContainer = Box.renderPrettySync(pipe(
  Container.make({ width: 80, padding: 2 }, (ctx) =>
    Box.vcat([
...
export const outContainerFlex = Box.renderPrettySync(pipe(
  Container.make({ width: 80, padding: 1 }, (ctx) =>
    Box.vcat([
      Flex.row(
        [
...
export const gridCards = ["Alpha", "Beta", "Gamma", "Delta", "Epsilon", "Zeta"].map((name) => Box.vcat([Box.text(`■ ${name}`), Box.text("  Item card")], Box.left).pipe(Box.pad(0, 1), Box.minWidth(22), Box.border("rounded")))
export const outGrid = Box.renderPrettySync(pipe(
  Grid.make(gridCards, { cols: 3, colWidth: 26, gap: [1, 1] }),
  Box.border()
))
...
const cards = ["Alpha", "Beta", "Gamma", "Delta", "Epsilon", "Zeta"]
  .map((name) =>
    Box.vcat([Box.text(`■ ${name}`), Box.text("  Item card")], Box.left)
      .pipe(Box.pad(0, 1), Box.minWidth(22), Box.border("rounded"))
  )

const grid = Grid.make(cards, { cols: 3, colWidth: 26, gap: [1, 1] })
```
