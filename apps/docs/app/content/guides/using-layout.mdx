...
export const outRow1 = Box.renderPrettySync(
  pipe(
    Flex.row(
      [
...
export const outRow2 = Box.renderPrettySync(
  pipe(
    Flex.row(
      [
        Flex.fill(
...
export const outFill = Box.renderPrettySync(
  pipe(
    Flex.row(
      [
...
export const outCol = Box.renderPrettySync(
  pipe(
    Flex.col(
      [
        Flex.fixed(
          Box.text("┄ Header ┄").pipe(Box.alignHoriz(Box.center1, 65))
        ),
        Flex.grow(Box.text("Body content goes here...").pipe(Box.pad(1, 2))),
        Flex.fixed(
          Box.text("┄ Footer ┄").pipe(Box.alignHoriz(Box.center1, 65))
        ),
      ],
      10
    ),
...
```typescript
const page = Flex.col( [ Flex.fixed(Box.text("┄ Header
┄").pipe(Box.alignHoriz(Box.center1, 65))), Flex.grow(Box.text("Body content
goes here...").pipe(Box.pad(1, 2))), Flex.fixed(Box.text("┄ Footer
┄").pipe(Box.alignHoriz(Box.center1, 65))), ], 10 )
```
...
export const outContainer = Box.renderPrettySync(
  pipe(
...
export const outContainerFlex = Box.renderPrettySync(
  pipe(
...
export const gridCards = [
  ...
  Box.vcat([Box.text(`■ ${name}`), Box.text("  Item card")], Box.left).pipe(
    Box.pad(0, 1),
    Box.minWidth(17),
    Box.border("rounded")
  )
);
export const outGrid = Box.renderPrettySync(
  pipe(
...
const cards = ["Alpha", "Beta", "Gamma", "Delta", "Epsilon", "Zeta"].map(
  (name) =>
    Box.vcat([Box.text(`■ ${name}`), Box.text(" Item card")], Box.left).pipe(
      Box.pad(0, 1),
      Box.minWidth(17),
...
export const outGridAuto = Box.renderPrettySync(
  pipe(Grid.auto(gridCards, 50, { minColWidth: 20 }), Box.border())
);
...
export const outComposed = Box.renderPrettySync(
  pipe(
    Container.make({ width: 65, padding: 1 }, (ctx) => {
      const cols = 3;
      const gap = 1;
      ...
      const kpis = Grid.make(
        [
          Box.vcat([Box.text("Users"), Box.text("1,247")], Box.left).pipe(
            Box.pad(0, 1),
            Box.minWidth(colWidth - 2),
            Box.border("rounded")
          ),
          Box.vcat([Box.text("Revenue"), Box.text("$48,290")], Box.left).pipe(
            Box.pad(0, 1),
            Box.minWidth(colWidth - 2),
            Box.border("rounded")
          ),
          Box.vcat([Box.text("Errors"), Box.text("23")], Box.left).pipe(
            Box.pad(0, 1),
            Box.minWidth(colWidth - 2),
            Box.border("rounded")
          ),
