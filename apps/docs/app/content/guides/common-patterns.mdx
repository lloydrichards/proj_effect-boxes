...
export const outStatus = Box.renderPrettySync(
  Box.vcat(
    [
      Box.hcat(
        [
          pipe(Box.text("✓ "), Box.annotate(Ansi.green)),
          pipe(Box.text("Tests passed"), Box.annotate(Ansi.green)),
        ],
        Box.top
      ),
      Box.hcat(
        [
          pipe(Box.text("✗ "), Box.annotate(Ansi.red)),
          pipe(Box.text("Build failed"), Box.annotate(Ansi.red)),
        ],
        Box.top
      ),
      Box.hcat(
        [
          pipe(Box.text("⚠ "), Box.annotate(Ansi.yellow)),
          pipe(Box.text("3 warnings"), Box.annotate(Ansi.yellow)),
        ],
        Box.top
      ),
      Box.hcat(
        [
          pipe(Box.text("ℹ "), Box.annotate(Ansi.blue)),
          pipe(Box.text("Running..."), Box.annotate(Ansi.blue)),
        ],
        Box.top
      ),
    ],
...
export const outProgress = Box.renderPrettySync(
  pipe(
    Box.hcat(
      [
...
export const outTable = Box.renderPrettySync(
  pipe(
    Box.vcat(
      [
        Box.punctuateH(
        ...
        Box.punctuateH(
          [
            Box.text("Alice"),
            Box.text("Engineer"),
            pipe(Box.text("Active"), Box.annotate(Ansi.green)),
          ].map((b) => pipe(b, Box.alignHoriz(Box.left, 16))),
          Box.top,
          Box.text("│")
        ),
        Box.punctuateH(
          [
            Box.text("Bob"),
            Box.text("Designer"),
            pipe(Box.text("Away"), Box.annotate(Ansi.yellow)),
          ].map((b) => pipe(b, Box.alignHoriz(Box.left, 16))),
          Box.top,
          Box.text("│")
        ),
        Box.punctuateH(
          [
            Box.text("Carol"),
            Box.text("Manager"),
            pipe(Box.text("Offline"), Box.annotate(Ansi.dim)),
          ].map((b) => pipe(b, Box.alignHoriz(Box.left, 16))),
          Box.top,
          Box.text("│")
        ),
...
const table = (headers: string[], rows: Box.Box<Ansi.AnsiStyle>[][]) => {
  ...
  const headerRow = Box.punctuateH(
    headers.map((h) =>
      pipe(
        Box.text(h),
        Box.annotate(Ansi.bold),
        Box.alignHoriz(Box.left, colWidth)
      )
    ),
    Box.top,
    Box.text("│")
  ...
  const separator = Box.text(
    "─".repeat(headers.length * colWidth + headers.length - 1)
  );
...
export const outKv = Box.renderPrettySync(
  pipe(
    Container.make({ width: 40, padding: 1 }, (ctx) =>
      Box.vcat(
        [
          Flex.row(
            [
              Flex.fixed(pipe(Box.text("Host:"), Box.annotate(Ansi.dim))),
              Flex.spacer(),
              Flex.fixed(Box.text("prod-01")),
            ],
            ctx.innerWidth
          ),
          Flex.row(
            [
              Flex.fixed(pipe(Box.text("Status:"), Box.annotate(Ansi.dim))),
              Flex.spacer(),
              Flex.fixed(pipe(Box.text("healthy"), Box.annotate(Ansi.green))),
            ],
            ctx.innerWidth
          ),
          Flex.row(
            [
              Flex.fixed(pipe(Box.text("Uptime:"), Box.annotate(Ansi.dim))),
              Flex.spacer(),
              Flex.fixed(Box.text("14d 3h")),
            ],
            ctx.innerWidth
          ),
          Flex.row(
            [
              Flex.fixed(pipe(Box.text("Load:"), Box.annotate(Ansi.dim))),
              Flex.spacer(),
              Flex.fixed(Box.text("0.42")),
            ],
            ctx.innerWidth
          ),
        ],
...
const label = (text: string) => pipe(Box.text(text), Box.annotate(Ansi.dim));
...
const dashboard = Container.make({ width: 65, padding: 1 }, (ctx) => {
  ...
  const header = Flex.row(
    [
      Flex.fixed(heading("Dashboard")),
      Flex.spacer(),
      Flex.fixed(label("v1.0")),
    ],
    ctx.innerWidth
  );

  // KPI cards in a grid
  const kpis = Grid.auto(kpiData.map(renderKpiCard), ctx.innerWidth, {
    minColWidth: 20,
  });
