...
export const outBasic = Box.renderPrettySync(pipe(
  Box.text("Hello"),
...
export const outAnnotated = Box.renderPrettySync(Box.vcat([
  pipe(Box.text("Success!"), Box.annotate(Ansi.combine(Ansi.bold, Ansi.green))),
