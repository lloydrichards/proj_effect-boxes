...
const program = Box.printBox(box).pipe(Effect.provide(Renderer.PlainRendererLive))
// Effect.runPromise(program) prints: Hello, Box!
```
...
const program = pipe(box, Box.render(), Effect.provide(Renderer.PlainRendererLive))
```
