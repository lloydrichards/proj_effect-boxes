...
```ts
export declare const combineAll: <T extends readonly Box<unknown>[]>(
  collection: T
...
const longText =
  "This is a very long sentence that needs to be wrapped to fit within a specific width";
...
```ts
export declare const minHeight: {
  (n: number): <A>(self: Box<A>) => Box<A>;
  <A>(self: Box<A>, n: number): Box<A>;
};
```
...
```ts
export declare const moveRight: {
  (n: number): <A>(self: Box<A>) => Box<A>;
  <A>(self: Box<A>, n: number): Box<A>;
};
```
...
```ts
export declare const printBox: <A>(
  self: Box<A>
