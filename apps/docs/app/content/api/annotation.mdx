...
```ts
export declare const createAnnotations: <A>(
...
```ts
export declare const isAnnotation: (
  value: unknown
) => value is Annotation<unknown>;
```
...
```ts
export declare const mapAnnotationData: <A, B>(
...
const cssAnnotation = Annotation.mapAnnotationData(redAnsi, (ansiCommands) => ({
...
```ts
export declare const cloneAnnotation: <A>(
  annotation: Annotation<A>
) => Annotation<A>;
```
...
```ts
export declare const extractAnnotationData: <A>(
