...
- They compose via `combineAnnotations` for layered styling

### Common tasks

- **Create** an annotation: `createAnnotation`, `empty`
- **Inspect**: `isAnnotation`, `getAnnotationData`
- **Transform**: `mapAnnotationData`, `combineAnnotations`
...
```ts
export declare const createAnnotation: <A>(data: A) => Annotation<A>
```
...
// Apply to box
const buttonWithTooltip = Box.text("Click me").pipe(Box.annotate(tooltip))
```
...
```ts
export declare const createAnnotations: <A>(dataArray: readonly A[]) => Annotation<A>[]
```
...
```ts
export declare const empty: Annotation<never>
```
...
```ts
export declare const isAnnotation: (value: unknown) => value is Annotation<unknown>
```
...
```ts
export declare const AnnotationTypeId: typeof AnnotationTypeId
```

### AnnotationTypeId (type alias)
...
```ts
export declare const mapAnnotationData: <A, B>(annotation: Annotation<A>, mapper: (data: A) => B) => Annotation<B>
```
...
```ts
export declare const cloneAnnotation: <A>(annotation: Annotation<A>) => Annotation<A>
```
...
```ts
export declare const extractAnnotationData: <A>(annotations: readonly Annotation<A>[]) => A[]
```
...
```ts
export declare const getAnnotationData: <A>(annotation: Annotation<A>) => A
```
